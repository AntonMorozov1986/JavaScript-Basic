...
const productClassEl = document.querySelector('div.products');
...
function clickHandler(event) {
    ...
    productClassEl.textContent = '';
    //в showCategory надо передать строку с типом категории, тип берите
    //из атрибута data-type у кнопки, по которой кликнули.

    //Вопрос:
    //Игорь, насколько хорошо делать переменную для категории продукта или это лишнее и нормально будет
    //сразу выражение "event.target.dataset.type" вставить в аргумент функции showCategory?
    //Спасибо.
    let categoryOfProducts = event.target.dataset.type;
    showCategory(categoryOfProducts);
...
function showCategory(category) {
    ...
    productClassEl.insertAdjacentHTML('afterbegin', productMarkup);
}
...
function getProductMarkup(product) {
    ...
    for (let productItem of product) {
        productMarkup += `<div class="product"> 
